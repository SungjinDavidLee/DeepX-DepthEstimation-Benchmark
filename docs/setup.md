...
DX-AllSuite 배포판의 Model Zoo에 사전 컴파일된 `.dxnn` 파일이 포함되어 있다. 표준 모델을 사용하는 경우 별도 컴파일이 필요하지 않다.

```
dx_app/assets/models/
```

이 디렉토리에 깊이 추정(`depthanythingv2-*`, `fastdepth`, `scdepthv3`), 검출(`yolov8-*`, `yolo11-*`, `yolo26-*`), 분할(`*-seg`, `deeplabv3plus-*`, `segformer`, `bisenetv2`, `pidnet-s`, `fastsam-s`) 계열이 포함되어 있다.
...
## 2. RealSense D405 연결 (선택)

카메라 입력을 사용하는 실사용 측정에 필요하다. 벤치마크 자체에는 필요하지 않다.
...
`trtexec`가 JetPack에 포함되어 있어 추가 설치가 필요하지 않다.

```bash
ls /usr/src/tensorrt/bin/trtexec
```

ONNX는 외부에서 확보한다. 이 장비에는 `pip`이 설치되어 있지 않아 export를 수행할 수 없다.

빌드와 측정 명령은 [methodology.md](methodology.md)를 참조한다.
