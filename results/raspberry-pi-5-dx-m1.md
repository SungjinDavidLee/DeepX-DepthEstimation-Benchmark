...
호스트 메모리 7.87 GB, 커널 6.18.39+rpt-rpi-2712.

---

## 2. 객체 검출 — yolov8-n, 640×640
...
## 3. 인스턴스 분할 — yolov8-n-seg, 640×640
...
cpu_0은 patch embedding conv를 수행하여 `[1,588,16,16]`을 산출한다. npu_0 출력은 `[1,32,128,128]`이며 cpu_1이 이를 최종 해상도로 변환한다. NPU 메모리 사용량 76.4 MB.
...
ViT-S와 동일하되 npu_0 출력이 `[1,64,128,128]`이다. NPU 메모리 사용량 170.5 MB.
...
## 6. 클래스 무관 분할 — FastSAM-s, 1024×1024
...
## 7. 참고 — `DXRT_DYNAMIC_CPU_THREAD` 비교
...
ON에서 코어 간 편차가 줄었다. 본 문서의 다른 측정값은 모두 미설정 상태다.

---

## 8. 측정하지 않은 항목

- **전력** — Raspberry Pi 5에 내장 전력 센서가 없고 외부 계측기를 확보하지 못했다. `dxrt-cli --status`는 NPU 전압(750 mV)과 온도만 보고하며 전류는 제공하지 않는다.
